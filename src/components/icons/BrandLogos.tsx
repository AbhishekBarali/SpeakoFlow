...
import speakoFlowMiniMark from "@/assets/speakoflow-mini.png";
...
/**
 * SpeakoFlow's own mark, used by the SpeakoFlow Mini cleanup model: the app
 * icon itself, so a model of ours reads as ours in a catalog of Qwen/Gemma/
 * NVIDIA logos.
 *
 * Unlike every other brand here this is artwork, not a `currentColor` glyph —
 * it carries its own gradient tile, so `BRAND_TILES.speakoflow` deliberately
 * draws no background behind it. It fills whatever tile the card gives it
 * (36px and 40px today), which is why the asset ships at 128px: enough for a 3x
 * display without shipping the full-resolution master. Regenerated by
 * `Logo/apply-icon-to-app.mjs`.
 */
export const SpeakoFlowLogo: React.FC<{ className?: string }> = ({
  className,
}) => (
  <img
    src={speakoFlowMiniMark}
    alt=""
    aria-hidden
    draggable={false}
