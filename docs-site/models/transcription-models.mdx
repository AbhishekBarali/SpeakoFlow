...
description: "The full catalog of speech-to-text models SpeakoFlow can download and run locally, with size, language coverage, streaming, and translation for each. Also covers older models, downloads, and deleting models."
icon: "microchip"
---

Every model runs on your machine. They trade accuracy for speed, so pick the one that fits your hardware and languages.

Open the catalog from _Settings → Dictation → Change model_.
...
<Note>
  Streaming models can transcribe as you speak, which is what [live
  transcription](/dictation/live-transcription) needs. Everything else
  transcribes in one pass when you stop talking.
</Note>
...
<Tip>
  If a download looks stuck at 0%, it is usually still negotiating the redirect
  to Hugging Face's CDN. Give it a few seconds before cancelling.
</Tip>
