...
description: "Every command-line flag and environment variable SpeakoFlow accepts on Windows, macOS, and Linux, including the remote-control flags that start, stop, or cancel a recording on an already-running instance."
icon: "terminal"
---
...
<Note>
  Flags are runtime-only overrides. They do not modify your saved settings, so
  leaving the flag off next launch returns you to whatever is configured in
  Settings.
</Note>
