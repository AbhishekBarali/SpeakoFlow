...
description: "A two-minute SpeakoFlow quickstart: grant microphone and accessibility permissions, pick a local speech-to-text model, optionally add a local assistant model, and dictate your first sentence."
icon: "rocket"
---

[Install SpeakoFlow](/start/install), then run the setup wizard. It takes about two minutes.
...
<Steps>
  <Step title="Grant permissions" icon="lock">
    ...
    Skip them and you'll be asked again the next time SpeakoFlow starts. See [Troubleshooting](/reference/troubleshooting) if a permission gets stuck.
...
Every shortcut is rebindable in _Settings → General → Shortcuts_.
...
Next: [how SpeakoFlow works](/start/how-it-works).
