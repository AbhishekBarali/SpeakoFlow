...
description: "The SpeakoFlow assistant panel is a floating always-on-top chat you open with a hotkey. Learn its shortcuts, how to ask by voice or text, attach images and screen regions, and resize the panel."
icon: "comments"
---
...
Both are rebindable in _Settings → Assistant → Shortcuts_.
...
| ---------------------- | ----------------------------------------------------------------------------------------------------------------------------------------- |
| **Ask by voice**       | Hold _Ask the assistant_ and talk. Your transcript appears, then the answer streams in token by token.                                    |
...
<Tip>
  Type `/summarize` to compact the whole conversation into a summary that
  replaces the transcript. It is the one slash command.
</Tip>
...
Three knobs in _Settings → Assistant → Panel appearance_, with a live preview as you change them.
