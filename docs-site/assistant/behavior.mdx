...
description: "Control how much conversation context the SpeakoFlow assistant sends each turn, whether long chats get summarized instead of dropped, and how long its replies run."
icon: "sliders"
---

Tune how the assistant carries a conversation, in _Settings → Assistant → Behavior_.
...
<Tip>
  You can compact a conversation on demand by typing `/summarize` in the panel.
  That one replaces the visible transcript with its summary, rather than working
  quietly in the background.
</Tip>

<Note>
  This is short-term, per-conversation context. For durable facts the assistant
  remembers across chats, see [Personal memory](/personalize/memory), a separate
  opt-in feature.
</Note>
