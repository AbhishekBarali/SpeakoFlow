---
title: "Screen vision"
description: "Ask the SpeakoFlow assistant about what is on your screen. Covers the three screen access modes, when the frame is captured, image quality per provider, and the macOS Screen Recording permission."
icon: "eye"
---

Ask about what is on your screen and the assistant answers with that context. It only looks when you let it.

Set it up in _Settings → Assistant → Screen vision_. You need a vision-capable model. Most curated local models qualify (the catalog marks them "Vision"), as do the mainstream cloud multimodal models.
...
<Warning>
  **macOS.** Screen vision needs macOS Screen Recording permission. Grant it in
  *System Settings → Privacy & Security → Screen Recording*, then restart
  SpeakoFlow for it to take effect.
</Warning>
