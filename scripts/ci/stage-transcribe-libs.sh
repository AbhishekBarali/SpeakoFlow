...
# them. This script copies them, as a co-located set, next to where the
# `speakoflow` binary will find them at runtime:
#   - the loader resolves `libtranscribe` via the $ORIGIN-relative rpath baked
#     into `speakoflow` (see src-tauri/build.rs);
...
#
# Usage: stage-transcribe-libs.sh <src-lib-dir> <dest-lib-dir>
#   <src-lib-dir>  the transcribe-cpp-sys install lib dir (contains
#                  libtranscribe.so* + libggml*.so* + transcribe-link.json)
#   <dest-lib-dir> where to place them (e.g. an AppImage's usr/lib)
set -euo pipefail

SRC="${1:?usage: stage-transcribe-libs.sh <src-lib-dir> <dest-lib-dir>}"
DEST="${2:?usage: stage-transcribe-libs.sh <src-lib-dir> <dest-lib-dir>}"
...
# symlinks so the package gets real files. `|| true` guards an unmatched glob;
# the verification below is the real gate.
