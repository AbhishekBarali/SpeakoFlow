//! Model capability probing — the seam between the model UI and "what can this
//! GGUF actually do".
//!
...
//! *before* a load (and, for the bundled recommended set, the same values ship
//! in `catalog.json`), so listing can show them honestly ahead of a load.
//!
...
//!
//! Ported from Handy (`src-tauri/src/managers/model_capabilities.rs`) so future
//! Handy updates diff cleanly (PLAN.md Session 3 / 7).
...
pub trait CapabilityProber: Send + Sync {
    /// Probe a GGUF already on disk (custom-dir scans, post-download).
...
mod tests {
    ...
    #[test]
    fn parakeet_streaming_unknown_when_key_absent() {
        // "Never guess" — parakeet streaming is inferred by the native loader,
        // so an absent key must remain None (settled post-load), not false.
        let meta = meta_with(vec![(
            "general.architecture",
            GgufValue::String("parakeet".into()),
        )]);
        assert_eq!(
            CapabilityProbe::from_metadata(&meta).supports_streaming,
            None
        );
    }
}
