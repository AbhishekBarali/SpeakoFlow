...
pub fn init() {
    PORTABLE_DATA_DIR.get_or_init(|| {
        ...
        let is_portable = if is_valid_portable_marker(&marker_path) {
            ...
            // Migration: v0.8.0 (Handy) created an empty marker file. If we
            // find an empty/invalid marker alongside an existing Data/ dir,
            // this is a real portable install — upgrade the marker in place.
...
fn is_valid_portable_marker(path: &std::path::Path) -> bool {
    std::fs::read_to_string(path)
        .map(|s| {
            let trimmed = s.trim();
            trimmed.starts_with(PORTABLE_MARKER_MAGIC) || trimmed.starts_with("Handy Portable Mode")
        })
        .unwrap_or(false)
}
...
mod tests {
    ...
    fn test_valid_magic_string_enables_portable() {
        let dir = std::env::temp_dir().join("speakoflow_test_valid");
    ...
    fn test_empty_file_does_not_enable_portable() {
        let dir = std::env::temp_dir().join("speakoflow_test_empty");
        std::fs::create_dir_all(&dir).unwrap();
        let marker = dir.join("portable");
    ...
    fn test_wrong_content_does_not_enable_portable() {
        let dir = std::env::temp_dir().join("speakoflow_test_wrong");
        std::fs::create_dir_all(&dir).unwrap();
        let marker = dir.join("portable");
        let mut f = std::fs::File::create(&marker).unwrap();
    ...
    fn test_legacy_empty_marker_without_data_dir_does_not_enable_portable() {
        ...
        let dir = std::env::temp_dir().join("speakoflow_test_legacy_no_data");
        std::fs::create_dir_all(&dir).unwrap();
        let marker = dir.join("portable");
        std::fs::File::create(&marker).unwrap();
        assert!(!is_valid_portable_marker(&marker));
    ...
    fn test_magic_string_with_whitespace_enables_portable() {
        let dir = std::env::temp_dir().join("speakoflow_test_ws");
        std::fs::create_dir_all(&dir).unwrap();
        let marker = dir.join("portable");
        let mut f = std::fs::File::create(&marker).unwrap();
        write!(f, "  {}\n", PORTABLE_MARKER_MAGIC).unwrap();
        assert!(is_valid_portable_marker(&marker));
        std::fs::remove_dir_all(dir).unwrap();
    }
}
