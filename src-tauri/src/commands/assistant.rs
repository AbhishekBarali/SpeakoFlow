...
/// Play one WAV chunk generated by the in-webview Kokoro model through the
/// native audio backend. The assistant HUD intentionally never takes keyboard
/// focus, so relying on HTMLAudio playback is fragile on Linux/WebKit (it may
/// be treated as background autoplay). Native playback also honours the
/// selected output device, app volume, and the shared cancellation epoch.
#[tauri::command]
#[specta::specta]
pub async fn assistant_play_local_tts_chunk(app: AppHandle, audio: Vec<u8>) -> Result<(), String> {
    if audio.is_empty() {
    ...
    let task_result = tauri::async_runtime::spawn_blocking(move || {
        crate::tts::play_audio_bytes(audio, device, volume, epoch).map_err(|e| e.to_string())
    })
    .await;
    let _ = app.emit("assistant-tts-playing", false);
    let result = task_result.map_err(|e| format!("local TTS playback task failed: {e}"))?;
    result
}
