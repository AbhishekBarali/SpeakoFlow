...
- `settings.rs` - Application settings management (recent additions: `sound_theme`; the experimental AI-Correction fields `post_process_enabled` / `post_process_tone` / `post_process_timeout_secs` / `post_process_unload_timeout` — cleanup keeps its **own** engine-residency policy, separate from the assistant's `local_llm_unload_timeout`; and the `experimental_enabled` gate that hides in-development features). The built-in provider declares `supports_structured_output: true`: llama.cpp compiles the JSON schema into a decoding grammar, which is what stops small chat models from narrating their plan instead of returning the cleaned transcript (measured on Gemma 4 E2B: 400 tokens of monologue without it, a correct 48-token answer with it)
- `managers/local_llm.rs` - Bundled llama.cpp sidecar. **Two engine instances, one per `LlmRole`**: `Assistant` (port 11435, vision projector, full context) and `Cleanup` (port 11436, no projector, context capped at `CLEANUP_CONTEXT_SIZE`, `LLAMA_ARG_THINK_BUDGET=0`, and `-ngl 0` for models ≤ `CLEANUP_CPU_MAX_BYTES` because CPU loads ~2x faster and skips the one-time 13–20s GPU shader compile). Sharing one engine meant a different model on either side evicted the other on every use. `warm_up()` sends a synthetic prefill so page-in and GPU pipeline compilation happen during recording rather than on the user's first cleanup
...
- `agents/` - Managed coding-agent sessions (Claude Code today, driven over its bidirectional `stream-json` control protocol) with a rolling per-session digest, spoken status answers, and permission prompts that refuse voice approval for destructive commands. **Being reworked onto the Agent Client Protocol (ACP) so one client covers Kiro CLI, Claude Code, Codex, Gemini, Copilot, and ~40 other agents — read [docs/voice-agent-control.md](docs/voice-agent-control.md) before touching this area**, and use `scripts/acp-probe.mjs` to check what an agent actually does rather than trusting docs
