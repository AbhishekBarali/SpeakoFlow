...
- `settings.rs` - Application settings management (recent additions: `sound_theme`; the experimental AI-Correction fields `post_process_enabled` / `post_process_tone` / `post_process_timeout_secs` / `post_process_unload_timeout` — cleanup keeps its **own** engine-residency policy, separate from the assistant's `local_llm_unload_timeout`; and the `experimental_enabled` gate that hides in-development features). `assistant_enabled` (default true) is the master switch for the assistant: off means the panel window is never created at launch (and is destroyed, not hidden, when it is turned off), its two bindings — `assistant::ASSISTANT_BINDINGS`, enforced in `shortcut/handler.rs` and unregistered by `set_assistant_enabled` — do nothing, and Profiles/Memory disappear from Settings. "Generate with Flow" and AI Correction deliberately survive it: they share `assistant_provider_id` / `assistant_models`, so the brain picker stays visible. The built-in provider declares `supports_structured_output: true`: llama.cpp compiles the JSON schema into a decoding grammar, which is what stops small chat models from narrating their plan instead of returning the cleaned transcript (measured on Gemma 4 E2B: 400 tokens of monologue without it, a correct 48-token answer with it)
