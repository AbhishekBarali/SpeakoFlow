...
**Cloud transcription latency is connection setup, not upload.** Measured against OpenRouter from Nepal (`stt_cloud_bench`, all `#[ignore]`d): WAV encoding is 12 ms and client construction 0.4 ms, both irrelevant. Latency does **not** scale with payload — 2 s of audio (62 kB) took 1.16 s and 10 s (312 kB) took 1.43 s on the same warm connection — so the app deliberately keeps sending uncompressed WAV; an MP3/Opus encoder would add a dependency and buy nothing. What did matter was that `block_on_request` built a **fresh thread, runtime, and `reqwest::Client` per request**, which makes keep-alive impossible: a pooled connection belongs to the runtime whose I/O driver services it, so tearing that runtime down discarded the connection. There is now one process-lifetime multi-threaded runtime (one worker, which keeps polling the I/O driver between dictations so an idle connection survives the gap) plus a client cache keyed on base URL + key + timeout, with a 300 s idle timeout and TCP keep-alive. An interleaved 5-round A/B measured median 2.65 s fresh against 1.93 s pooled — ~715 ms, 27% — with all five pooled samples beating their fresh counterpart; a later run measured only 3%, because by then the OS DNS cache and TLS session tickets had made even a "fresh" handshake cheap. So the honest range is 3–27% depending on how cold the machine's network state is, and reuse is never worse. `prewarm_cloud_stt` opens the connection at recording start so the setup overlaps with speech; it must use an **authenticated GET against the provider's own API path**, because an anonymous `HEAD` at the origin returns `200 keep-alive` and still left nothing the transcription POST would reuse (measured as no gain across three rounds). The residue beyond all this — the first request of a burst staying slower than later ones, unrecoverable by pre-opening a socket — is the provider's own routing and model warm-up plus round-trip distance. Only a realtime WebSocket removes it, by uploading during speech; that is why ElevenLabs `scribe_v2_realtime` feels immediate and OpenRouter, which has no realtime endpoint, cannot. Latency on this path varies by a second run to run, so measure with repeats and medians — single samples have already produced two wrong conclusions here.
