...
**"Translate to English" is a local-engine feature.** It maps onto Whisper's translate task and no cloud transcription endpoint wired up here accepts an equivalent, so on `SttEngineMode::Cloud` the transcript comes back in the language spoken. `transcribe()` logs a `warn!` when the setting is on, and the completion line only says `(translated)` when the engine was actually asked _and_ can do it (`ModelInfo::supports_translation`) — reading the setting alone made the log assert a translation that never happened, which is how a Japanese transcript came out of a run logged as "translated".
